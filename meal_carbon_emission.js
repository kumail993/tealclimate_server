...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const mapArray = req.body.mealDetails;
       
        let total_kgco2e = 0;
        let successfulInsertions = -1;
        let ghg_conversion_factor=0;
        mapArray.map(element => {
            const query = 'SELECT * FROM meal_database WHERE category = $1 AND stage = $2';
            const values = [element.category,element.stage];
            ...
            db.query(query, values, (error, result) => {
                if (error) {
                    ...
                    result.rows.forEach(data => {
                        let kgco2e = data.ghg_conversion_factor * element.numberOfMeals;
